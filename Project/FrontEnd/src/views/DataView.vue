<template>
  <div class="intro">
    <span class="intro-title">数据展示</span>
    ...
    <div class="choices">
      <a-form :form="form" ref="form" :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }" style="font-size: 20px;">
        <a-form-item label="楼宇" :label-col="{ span: 8 }" :wrapper-col="{ span: 4 }">
          <a-select v-model="form.selectedBuilding" @update:value="updateSelectedBuilding" placeholder="请选择楼宇"
            @change="getDevices" style="width: 200px;">
            <a-select-option v-for="building in buildings" :key="building[0]" :label="building[0]"
              :value="building[0]"></a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item label="设备" :label-col="{ span: 8 }" :wrapper-col="{ span: 4 }">
          <a-select v-model="form.selectedDevice" @update:value="updateSelectedDevice" placeholder="请选择设备"
            style="width: 200px;">
            <a-select-option v-for="device in devices" :key="device[0]" :label="device[0]"
              :value="device[0]"></a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item label="起始日期" :label-col="{ span: 8 }" :wrapper-col="{ span: 4 }">
          <a-date-picker v-model="form.startDate" @update:value="StartDime" show-time format="YYYY-MM-DD HH"
            placeholder="选择起始日期" :disabled-date="disabledDate"></a-date-picker>
        </a-form-item>
        <a-form-item label="结束日期" :label-col="{ span: 8 }" :wrapper-col="{ span: 4 }">
          <a-date-picker v-model="form.endDate" @update:value="EndTime" show-time format="YYYY-MM-DD HH"
            placeholder="选择结束日期" :disabled-date="disabledDate"></a-date-picker>
        </a-form-item>
        <a-form-item>
          <a-button type="primary" @click="getData"
            style="margin-left: 150px;height: 40px;font-size: 18px;">查询数据</a-button>
        </a-form-item>
      </a-form>
    </div>
